...
static bool g_upload_started = false;
static bool g_upload_is_binary = false;
static bool g_upload_reject = false;
...
void handleUpload(ESP8266WebServer &server) {
  ...
  if (ESP.getFreeHeap() < MAX_HEAP_REQUIRED) {
    doc["success"] = false;
    doc["message"] = "Insufficient memory. Use streaming upload for large files.";
    sendJson(server, doc);
    return;
  }

  String hexContent = server.arg("plain");
  int bytesWritten = writeHexToEEPROM(hexContent);
  
  if (bytesWritten > 0) {
    doc["success"] = true;
    doc["bytesWritten"] = bytesWritten;
    doc["message"] = "HEX file programmed successfully";
  } else {
    doc["success"] = false;
    doc["message"] = "Failed to program HEX file";
  }
...
void handleDump(ESP8266WebServer &server) {
  ...
  for (uint16_t addr = 0; addr < EEPROM_SIZE; addr += BUFFER_SIZE) {
    ...
    // Change this line: use sendContent instead of sendContent_P
    server.sendContent(reinterpret_cast<const char*>(buffer), BUFFER_SIZE);
    
    // Small delay every 1KB to prevent watchdog timeout
    if (addr % 1024 == 0) {
      delay(1);
    }
  }
 ...
 */
/**
 * Upload stream handler - FIXED VERSION
 */
/**
 * Upload stream handler - FIXED VERSION
 */
void handleUploadStream() {
  ...
  switch (upload.status) {
    case UPLOAD_FILE_START: {
      ...
      // Disable write protection during upload
      setWriteProtect(false);

      // Determine total expected bytes from various sources
      uint32_t total = 0;
      
      // Try URL parameter 'size'
      String qsize = g_server->arg("size");
      if (!qsize.isEmpty()) {
        total = static_cast<uint32_t>(qsize.toInt());
        Serial.printf("Using URL size param: %u\n", total);
      }
      
      // Try X-File-Size header
      if (total == 0) {
        String xfs = g_server->header("X-File-Size");
        if (!xfs.isEmpty()) {
          total = static_cast<uint32_t>(xfs.toInt());
          Serial.printf("Using X-File-Size header: %u\n", total);
        }
      }
      
      // Try Content-Length header (this includes multipart overhead)
      if (total == 0) {
        String clen = g_server->header("Content-Length");
        if (!clen.isEmpty()) {
          total = static_cast<uint32_t>(clen.toInt());
          Serial.printf("Using Content-Length header: %u\n", total);
        }
      }
      
      // Default fallback - use file size from client if available
      if (total == 0 && upload.totalSize > 0) {
        total = upload.totalSize;
        Serial.printf("Using upload.totalSize: %u\n", total);
      }
      
      // Final fallback
      if (total == 0) {
        total = 32768; // 32KB default
        Serial.printf("Using default size: %u\n", total);
      }
      
      setExpectedTotalBytes(total);
      
      // Reset upload state
      g_upload_started = false;
      g_upload_is_binary = false;
      g_upload_reject = false;
      
      Serial.printf("Expected total bytes: %u\n", total);
      break;
    }

    case UPLOAD_FILE_WRITE:
      if (upload.currentSize > 0) {
        Serial.printf("UPLOAD WRITE: currentSize=%d, totalSize=%d\n", 
                     upload.currentSize, upload.totalSize);
        
        // Debug: show first few bytes of chunk - FIXED min() call
        Serial.print("Chunk data (first 16 bytes): ");
        int bytesToShow = (upload.currentSize < 16) ? upload.currentSize : 16;
        ...
        Serial.printf("Parser stats: bytesWritten=%d, linesProcessed=%d\n", 
                     getTotalBytesWritten(), getTotalLinesProcessed());
    ...
    case UPLOAD_FILE_END:
      ...
      processHexChunk("", 0); // Force flush any buffered data
      
      // Re-enable write protection
      setWriteProtect(true);
      
      Serial.printf("FINAL STATS: Parser wrote %d bytes, EEPROM wrote %u bytes\n",
                   getTotalBytesWritten(), getBytesWrittenProgress());
...
/**
 * Upload stream completion handler - FIXED VERSION
 */
void handleUploadStreamComplete() {
  if (!g_server) return;
  ...
  uint32_t parserBytes = getTotalBytesWritten();
  uint32_t eepromBytes = getBytesWrittenProgress();
  uint32_t expectedBytes = getBytesToWrite();
  
  Serial.printf("=== UPLOAD COMPLETE ===\n");
  Serial.printf("Parser processed: %u bytes\n", parserBytes);
  Serial.printf("EEPROM written: %u bytes\n", eepromBytes);
  Serial.printf("Expected: %u bytes\n", expectedBytes);
  Serial.printf("Lines processed: %d\n", getTotalLinesProcessed());
  
  // Determine success based on reasonable criteria
  bool success = false;
  String message;
  
  if (parserBytes > 0 && eepromBytes > 0) {
    // If we got at least 80% of expected data, consider it success
    if (expectedBytes > 0 && eepromBytes >= expectedBytes * 0.8) {
      success = true;
      message = "HEX file programmed successfully";
    } else if (expectedBytes == 0 && eepromBytes > 0) {
      success = true;
      message = "HEX file programmed successfully (size unknown)";
    } else {
      success = false;
      message = String("Partial upload: ") + eepromBytes + "/" + expectedBytes + " bytes";
    }
  } else {
    success = false;
  ...
  doc["parsedBytes"] = parserBytes;
  doc["linesProcessed"] = getTotalLinesProcessed();
  doc["bytesTotal"] = expectedBytes;
  ...
  Serial.println("Upload response sent to client");
}

/**
 * Upload stream completion handler
 */
...
void register_web_routes(ESP8266WebServer &server) {
  ...
  // Upload operations
  server.on("/upload", HTTP_POST, []() { handleUpload(*g_server); });
  server.on("/upload_stream", HTTP_POST, handleUploadStreamComplete, handleUploadStream);
