...
bool writeToEEPROM(uint16_t address, uint8_t data[], int length) {
  // CRITICAL FIX: Check if we're about to exceed EEPROM size
   if (ESP.getFreeHeap() < 2000) {
    Serial.printf("CRITICAL: Not enough memory for write (%d bytes free)\n", ESP.getFreeHeap());
    return false;
  }
...
while (bytesWritten < length) {
  // Remaining space in current EEPROM page
  int pageOffset = (address + bytesWritten) % EEPROM_PAGE_SIZE;
  int pageRemaining = EEPROM_PAGE_SIZE - pageOffset;

  // How many bytes we can safely write this transfer
  int maxForThisXfer = min(pageRemaining, I2C_MAX_DATA_PER_XFER);
  int bytesToWrite = min(maxForThisXfer, length - bytesWritten);

  Wire.beginTransmission(EEPROM_I2C_ADDRESS);
  Wire.write((uint8_t)((address + bytesWritten) >> 8)); // high addr
  Wire.write((uint8_t)((address + bytesWritten) & 0xFF)); // low addr
  Wire.write(data + bytesWritten, bytesToWrite);

  int res = Wire.endTransmission();
  if (res != 0) {
    Serial.printf("I2C write failed at 0x%04X res=%d\n", address + bytesWritten, res);
    return false;
  }

  // Wait until EEPROM finished its internal write cycle (poll ACK)
